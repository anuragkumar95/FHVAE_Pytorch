...
class BaseDataset(Dataset):
    def __init__(
        ...
        self.num_seqs = len(self.seq_keys)
...
class NumpySpeakerDataset(BaseDataset):
    def __init__(
        self,
        feat_scp: Path,
        len_scp: Path,
        ...
        for i, seg in enumerate(self.segs):
            spk = seg.seq.split('_')[0]
            if spk not in self.spk2seg_id:
                self.spk2seg_id[spk] = []
            self.spk2seg_id[spk].append(i)

        self.ds_ids = []
        np.random.seed(123)

        for spk in self.spk2seg_id:
            spk_ids = self.spk2seg_id[spk]
            spk_ids_train = np.random.choice(spk_ids, int(0.7*len(spk_ids)), replace=False)
            spk_ids_remain = [i for i in spk_ids if i not in spk_ids_train]
            spk_ids_val = np.random.choice(spk_ids_remain, int(0.15*len(spk_ids)), replace=False)
            spk_ids_test = [i for i in spk_ids if i not in spk_ids_train and i not in spk_ids_val]
            if split=='train':
                self.ds_ids.extend(spk_ids_train)
            if split=='val':
                self.ds_ids.extend(spk_ids_val)
            if split=='test':
                self.ds_ids.extend(spk_ids_test)
        self.split = split
        print(f"DS_LEN:{len(self.ds_ids)}")
        print(f"Num_SPK:{len(self.spk2idx.keys())}")

    def __len__(self, ):
        return len(self.ds_ids)

    def __getitem__(self, index):
        """Returns key(sequence), feature, and number of segments."""
        seg = self.segs[self.ds_ids[index]]
        idx = self.seq2idx[seg.seq]
        with open(self.seq_feats[idx], "rb") as f:
            feat = np.load(f)[seg.start : seg.end]
        feat = self.apply_mvn(feat)
        spk = self.spk2idx[seg.seq.split('_')[0]]
        return feat, spk
...
class NumpyDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            seq = self.idx2seq[index]
            seg_idx = [k for k, seg in enumerate(self.segs) if seg.seq == seq]
            feat = []
            with open(self.seq_feats[index], "rb") as f:
                FEAT = np.load(f)
            for seg_i in seg_idx:
                seg = self.segs[seg_i]
                feat_i = FEAT[seg.start : seg.end]
                feat_i = self.apply_mvn(feat_i) 
                feat.append(feat_i[np.newaxis, :])
            feat = np.concatenate(feat, axis=0)
            nsegs = len(seg_idx)

            #For val or test, this is an unknown index
            idx=-1
